...
## Full Knowledge Document — v1.9.9
...
`RMC_Handoff_v1.9.9.md`, and `RMC_IO_Map.md` into one readable account.

v1.9.9 supersedes v1.9.8 (kept frozen as a snapshot). It folds in the scheduler
evolution that landed between v1.9.8 and the mentor review — the weight arbiter,
adaptive batching, the per-bank row-lock, and the post-mentor residency rework
(TCAM admission + per-bank in-flight queues, RAW pause). Still docs-only: pkg frozen,
no RTL, sizing sweep pending.

---

## 0. Changes Since v1.9.8

All changes are in the scheduler / MC-core buffering path; the CIF, AMU, maintenance
engine, and DFI boundary are unchanged from v1.9.8. Detailed specs live in the scheduler
doc set (`scheduler_queue_arch.md`, `short_notes/scheduler_deep.md`,
`scheduler_bank_fsm.md`, `scheduler_staged_logic.md`, `scheduler_adaptive_batching.md`);
this document carries the consolidated narrative.

1. **Residency split (mentor).** The single-home model — a request squats a TCAM slot
   from arrival to retire (~118 tCK worst case) — is replaced by a two-stage split:
   TCAM becomes a **short-residency admission/classify station**, and the request then
   **evicts into a per-bank in-flight FIFO**. The TCAM is sized for burst+classify
   latency, not the whole latency window. **TCAM is not removed** — the read path still
   owns it as the classify/RAW engine; only residency shortens. (§3.2)
2. **RAW: pause, not bypass (mentor).** The early-ACT RAW-forwarding/bypass path is
   dropped. A read younger than a pending write to the same address simply **stalls at
   admission** (held in TCAM, not evicted) until the write drains. Rare case, not worth
   the reorder hardware. Supersedes the RAW Bypass Manager's forward/partial-merge path.
   (§3.2)
3. **Per-bank in-flight queues.** 16 per-bank FIFOs (not per-command) hold classified
   requests in arrival order; only the **head** is active in the command pipe (a bank
   decodes one command at a time). Each bank exposes one ready-bit per class
   (`can_pre/act/cas[16]`) to the arbiter — command-class selection preserved without
...
5. **Weight arbiter replaces pure SJF Stage-3.** Winner selection is now
   ...
   CAS↔ACT with a guardrail that never idles the DQ bus). (§3.3)
6. **Adaptive batching replaces the fixed bank-partition scheme.** The read-half/
   write-half rotation on a fixed `WINDOW_SIZE` is superseded by adaptive R/W batching —
   batch direction flips on accumulated gate-loss (≥BL2) or stall (≥tRAS+tRP). (§3.2, §3.3)
7. **Golden model is the cycle-accurate reference.** `tools/sched_model/sched_test.js`
   is the RTL reference, timed on the **DDR5-4800B** bin (not the earlier toy 5 ns
   numbers). It implements both the window and queue-arch candidate sets (opt-gated);
   the queue-arch model runs 0 violations / 0 unscheduled and within ±2pt DQ-busy of the
   window model. (§5, §12)
8. **Deferred sizing/weights sweep.** Exact per-bank queue depth, ready-bank count,
...
**v1.9.9 — residency split.** The TCAM is no longer a request's home for its whole life.
It is now a **short-residency admission/classify station**: an incoming request searches
`{bg,bank}` (row-hit vs `open_row`), runs its RAW full-address compare, is classified
`NEED_PRE / NEED_ACT / NEED_CAS`, and is then **evicted into a per-bank in-flight FIFO**,
freeing the TCAM slot for the next arrival. This drops CAM depth/timing hard — the scarce
...
pause, not a bypass** (mentor review). The early-ACT forwarding/partial-merge scheme
described below is **superseded**: a read younger than a pending write to the same address
is simply **held at admission** (entry marked `blocked`, not evicted to its bank queue)
until the blocking write drains, then proceeds normally. No datapath forward, no reorder,
no merge — the rare RAW read eats the write's latency serially. The full-address compare
still happens where it always did (the admission search, reusing the pin-bit mechanism now
on the read side); its `raw_hit` output now gates **eviction** rather than driving a
bypass mux. Because RAW is resolved once at admission, the per-bank queues stay CAM-free.
Detail: `scheduler_queue_arch.md §3`.

*Superseded (v1.9.8 bypass model, retained for provenance):* the **RAW Bypass Manager**
had every incoming read search WR_TCAM in parallel with its own allocation (stage A,
combinational, zero penalty on a miss), then a stage-B mask-coverage check decide the
outcome — full hit (forward write-buffer data straight to the response path via the
**Merge Unit**'s 64-lane 2:1 mux), partial overlap (issue the DRAM fetch immediately with
no stall, tag `merge_pending`, combine WDB + DRAM data at return time), or miss/late-write
(pass the read through normally). A hit was only valid if the write's age was ≤ the read's
age. The **Hold-Forward** stage sat right after: exactly two response sources could produce
a result in the same cycle (the bypass path and a genuine DRAM return), so two hold slots
were the proven exact minimum and maximum. With the pause model there is only one response
source (DRAM), so the second hold slot and the forward mux are no longer required.
...
**v1.9.9 — adaptive batching supersedes the fixed Bank Partition scheme.** Turnaround is
now amortized by **adaptive R/W batching** at the arbiter: the scheduler runs a batch
direction (read or write) and flips it when accumulated *gate-loss* crosses `BL2` (a free
DQ slot was wasted skipping an opposite-direction CAS) or when it stalls for `≥ tRAS+tRP`
with opposite work waiting. This pays the `tRTW`/`tWTR` turnaround once per batch —
demand-driven, rather than on a fixed window boundary — and adapts the batch length to the
actual read/write mix. Detail: `scheduler_adaptive_batching.md`, `scheduler_deep.md §5`.

*Superseded (v1.9.8):* the **Bank Partition Controller** split the bank set into a
read-half and a write-half that rotated every `WINDOW_SIZE` cycles — since a read and a
write never shared a bank within a window, `tRTW`/`tWTR` didn't apply inside it and the
turnaround penalty was paid once at the rotation boundary. Adaptive batching achieves the
same turnaround amortization without statically partitioning the banks.
...
**v1.9.9 — what the stages read.** The five-stage skeleton and the single-writer rule are
unchanged, but two things shift: (a) the **candidate set** the picker scans is now the
**heads of the 16 per-bank queues** (one active request per bank), not a flat window of
TCAM entries — only the candidate source changes; the legality gates (`can_*`) and the
Stage-4 writeback are byte-identical either way (the golden model A/Bs the two); and
(b) Stage 3's pure-SJF winner is replaced by an explicit **weight arbiter** (below). The
gates read the same per-bank scoreboard timers (`next_cas/act/pre`), which are bank
properties the head consults — deeper queue entries just wait.
...
- **Stage 3 — Weight-Arbiter Winner Selection (v1.9.9, replaces pure SJF).** The winner is
  `argmax` of an explicit weight over the legal candidates:
  `weight(lane) = K·control[lane] + age[lane] + (lane==ACT ? servo_mod : 0)`, where
  `control` is the SJF ordering `CAS > ACT > PRE` (fixed), `age` counts waiting cycles
  (+1/cycle, 0 on win) so real starvation eventually dominates, and `servo_mod` is a
  **DQ-occupancy servo** that boosts ACT when the ready-CAS pool is thin and the DQ bus is
  about to free (prep more banks), and damps it when the pool is deep or tFAW is tight. A
  hard **guardrail** keeps CAS winning absolutely whenever the DQ is free and any CAS is
  legal — the bus is never idled. `K` scales control against age so SJF governs normal
  waits and aging only breaks genuine starvation. Ties resolve oldest-age, then BG-rotate
  (prefer a bank group different from the last activate: `tCCD_S=8 < tCCD_L=12`).
  Read/write direction is set by **adaptive batching** (§3.2) rather than fixed watermark
  crossings; the per-bank **row-lock** holds an open row while it still owes row-hit demand
  (`demand[bank][open_row] > 0`) and releases on drain or when `oldest_miss_age ≥ AGE_MAX`
  (hot-row starvation cap). The golden model currently implements the busy-first + oldest
  baseline of this arbiter; the full weighted form and its constants are the deferred sweep
  (§10). Detail: `scheduler_bank_fsm.md §4`, `scheduler_deep.md §6`.
...
| ~~Bank Partition FSM~~ (superseded v1.9.9 by adaptive batching) | 2 | 1 | Scheduler |
...
is needed later. Note the v1.9.9 RAW-pause rework retires the old RAW hold-forward
collision question entirely (only one response source remains).
...
Per the version-control roadmap, in priority order: run the **scheduler weights/sizing
sweep (OQ-20)** and close the **golden-model debt (OQ-21)** against
`tools/sched_model/sched_test.js` — the cycle-accurate reference, now timed on the
DDR5-4800B bin — then patch the stale LaTeX docs (pending bits, hardcoded buffer depths,
and the RAW partial-hit "degrade to DRAM" line are all known-stale — see BUG-02 through
BUG-07 in `rmc_version_control.md`), resolve OQ-19b with a Python address-hash optimizer
script, and only then begin the RTL skeleton: top-level module hierarchy and parameter
propagation. The pkg stays frozen until the sweep is done. Architecture is complete
through the v1.9.9 residency rework; RTL has not been started.
