...
f.text(416, 140, "aw_id  addr  sram_ptr  status  narrow", size=11, fill=MUTED)
...
cnote(60, 250, "narrow = size<full | wstrb!=1s | sub-64B")

# ---------------- W data -> WR_ACCUM (alloc on 1st beat) -> WD_SRAM ----------------
f.line(210, 322, 234, 322, arrow=True)
acc = f.logic(234, 296, 152, 62, "WR_ACCUM", "beats -> 64B + WSTRB", top=True)
f.line(386, 322, 400, 322, arrow=True)
sram = f.block(400, 296, 300, 120, "WD_SRAM")
f.text(416, 336, "512b/line + strobe", size=11, mono=False, fill=MUTED)
...
# narrow flag into WR_ACCUM
f.path("M311 252 V296", arrow=True, dashed=True)
cnote(318, 272, "narrow flag")
# back-pressure W (SRAM full -> WR_ACCUM stalls W)
f.path("M400 400 H311 V358", arrow=True, dashed=True)
cnote(150, 416, "SRAM full -> back-pressure W")
