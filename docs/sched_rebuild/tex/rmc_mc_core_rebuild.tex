...
\sig{timing_reg_file} & CSR only (MR\_Write has no write path to it) & all \\
...
MR\_Write (I28): MRW $\to$ optional verify-MRR $\to$ \sig{MR_WR_DONE}/\sig{MR_WR_ERROR}.
Shares the MRR sideband with MR\_Poll via a \sig{mrr\_busy} mutual-exclusion interlock
(not an arbiter: no requester tag, no response routing).
\textbf{\sig{timing_reg_file} sync after a successful write is a software
responsibility; no hardware path exists from MR\_Write to \sig{timing_reg_file}.}
...
bursts); slack vector A; backward scheduling; MR\_Write + shared \sig{mrr\_busy} interlock;
