...
per AXI write beat on the write path; one per distinct aligned 64-B memory
line touched on the read path), and returns AXI responses in order for each
AXID.  Read and write traffic use independent instances of the same
...
\begin{itemize}[noitemsep]
  ...
  \item An MC request packet carries a 512-bit payload on a 64-B byte-lane
        grid.  \textbf{Write path} (\texttt{AWSIZE = 0..6}): one MC packet per
        AXI write beat, carrying \texttt{WDATA[511:0]} and \texttt{WMASK[63:0]}
        (verbatim \texttt{WSTRB}); \texttt{exp\_pkts = AWLEN + 1}.  A narrow or
        unaligned write beat occupies its natural byte lanes and \texttt{WMASK}
        marks the valid bytes; \texttt{WMASK} not all-ones $\Rightarrow$ MC
        read-modify-write.  \textbf{Read path} (\texttt{ARSIZE = 0..6}): an MC
        packet is one distinct aligned 64-B line touched by the burst, several
        narrow AXI read beats may map to one MC packet, and \texttt{exp\_pkts}
        is the number of distinct 64-B lines (\S\ref{sec:splitter}), not
        \texttt{ARLEN + 1}.
  \item Narrow (\texttt{AxSIZE < 6}) and unaligned first transfers are legal
        AXI4 on both paths and are not runtime-checked; full-width
        (\texttt{AxSIZE = 6}, 64-B-aligned) transfers remain the common case.
        This replaces the earlier write-path assumption of a fixed
        \texttt{AWSIZE = 6} (see the Request Validator).
  \item An AXI transaction must not cross 4~KB, so a \emph{read} produces at
        most \texttt{MAX\_PKTS = 4\,KB / 64\,B = 64} MC packets.  A
        \emph{write} produces \texttt{AWLEN + 1} MC packets; the same bound
        \texttt{AWLEN + 1 $\le$ MAX\_PKTS = 64} is automatic for full-width
        writes (4~KB rule) and is a design-time assertion for narrow writes,
        so \texttt{PACKET\_NUMBER}, \texttt{completion\_bitmap}, and
        \texttt{exp\_pkts} widths are unchanged.  With runtime-configured
        \texttt{row\_size} of at least 1~KB, row crossings are at most three
        and affect each packet's address, not the packet count.
...
\texttt{DATA\_W} & 512: payload per MC packet (one AXI write beat; one returned 64-B read line feeding one or more AXI read beats)\\
...
This is the complete MC completion identity.  On the write path
\texttt{PACKET\_NUMBER} is the AXI write-beat index; on the read path it is
the 64-B line index within the transaction (\S\ref{sec:splitter}), not the
AXI beat index.
...
generates one MC packet per AXI write beat (\texttt{AWSIZE = 0..6}), up to 64
(\S\ref{sec:overview}), and the Write Packet FIFO forwards them to MC Core.
...
(\S\ref{sec:samelinehaz}).  The splitter emits one MC packet per distinct
64-B line touched by the read burst, up to 64, each carrying a distinct
\texttt{PACKET\_NUMBER} (\texttt{0..exp\_pkts-1}) that is the line index, not
the AXI beat index.
...
\texttt{ROB\_INDEX} while the Read Serializer emits \texttt{ARLEN + 1} AXI R
beats reconstructed from the returned lines (\S\ref{sec:read-serializer}):
beat \texttt{n}'s address is formed per \texttt{ARBURST}/\texttt{ARSIZE}
(\texttt{WRAP} relative to \texttt{WrapBoundary}) and mapped to its 64-B line
index (\texttt{0..exp\_pkts-1}), whose complete 64-B line at
\texttt{rd\_dbuf\_base + PACKET\_NUMBER} drives \texttt{RDATA} with no lane
realignment; several beats may share a line.  \texttt{RLAST} is asserted on
the final AXI beat (\texttt{beat\_index == ARLEN}), not on the final line.
Only after that beat is accepted and the \texttt{rd\_dbuf\_base} range
released may the Slot FIFO be popped and \texttt{ROB\_INDEX} returned to the
read Free List.  The serializer needs retained AXI burst metadata
(\texttt{ARLEN}, \texttt{ARSIZE}, \texttt{ARBURST}, normalized base /
\texttt{WrapBoundary}); its storage is subject to ROB reconciliation
(\S\ref{sec:read-completion}).  RD\_SRAM depth (hence
\texttt{RD\_SRAM\_ADDR\_W}), its handshakes and latency remain \textbf{TBD};
its 128-entry watermark is its own capacity control, not ROB occupancy
control.
...
is never re-checked.  This footprint is deliberately conservative
(outward-rounded to whole 64\,B lines) and is a distinct quantity from the
read's \texttt{exp\_pkts}: \texttt{exp\_pkts} is the \emph{exact} MC-line
count (\S\ref{sec:splitter}) that sizes the completion bitmap and the
RD\_SRAM allocation, whereas the hazard footprint need only be a safe
superset of the touched lines.  The two are not merged and not shared.
...
\noindent Up to \texttt{MAX\_PKTS = 64} request packets per transaction.  On
the write path one MC packet is one AXI write beat (any \texttt{AWSIZE}),
carrying \texttt{WDATA[511:0]} and \texttt{WMASK[63:0]} on the write-data
packet (\S\ref{sec:cifmc}); on the read path one MC packet is one distinct
64-B line touched by the burst, and multiple narrow AXI beats may map to it.
\texttt{PACKET\_NUMBER} is the Burst Splitter \texttt{pkt\_idx},
\texttt{0..exp\_pkts-1} --- the AXI beat index on the write path, the 64-B
line index on the read path.  There is no \texttt{beat\_cnt}, \texttt{size},
or \texttt{len} field; MC Core is unaware of
\texttt{AxSIZE}/\texttt{AxLEN}/\texttt{AxBURST}.  For read \texttt{WRAP}
bursts the CIF-side per-line address generation (\S\ref{sec:splitter}) uses
\texttt{WrapBoundary} as the normalized base before the address is mapped to
\texttt{daddr}.
...
\textbf{Field} & \textbf{Width} & \textbf{Meaning}\\
\midrule
\texttt{wdata} & 512 & one AXI write beat; a narrow/unaligned beat occupies its natural byte lanes\\
\texttt{wmask} & 64  & byte-enable, bit \texttt{i} $\leftrightarrow$ byte \texttt{i}, verbatim from AXI \texttt{WSTRB}.  \texttt{wmask} not all-ones (narrow, unaligned, or partial-strobe write) $\Rightarrow$ MC read-modify-write\\
\bottomrule
\end{tabular}
\end{center}
...
one per 64-B line.  A packet completing is not \texttt{RLAST}: on the read
path \texttt{RLAST} is a property of the reconstructed AXI beat stream
(\texttt{beat\_index == ARLEN}), asserted by the Read Serializer on the final
AXI beat once it has been accepted downstream, and is unrelated to which line
completes last.
...
\noindent \texttt{status} is per-packet.  \textbf{Read:} each line's mapped
\texttt{RESP} drives \texttt{RRESP} for every AXI R beat sourced from that
64-B line --- no aggregation.  \textbf{Write:} \texttt{resp\_status} is the worst-of over
...
\texttt{ARLEN + 1} R beats with \texttt{RRESP = SLVERR} and undefined data,
\texttt{rob\_last} on the final AXI beat (\texttt{beat\_index == ARLEN}); the
AXI error-beat count is \texttt{ARLEN + 1}, not \texttt{exp\_pkts}.
MC-detected faults on \emph{valid}
...
\begin{enumerate}
  \item \textbf{Response granularity.}  MC shall generate \emph{exactly one}
        ...
        on the interface --- on the read path CIF derives \texttt{RLAST} in
        the Read Serializer from the reconstructed AXI beat stream
        (\texttt{beat\_index == ARLEN}), independently of MC responses.
        Response packets for one transaction may arrive in any order; CIF's
        \texttt{completion\_bitmap} and
        \texttt{rd\_dbuf\_base + PACKET\_NUMBER} placement tolerate that.
