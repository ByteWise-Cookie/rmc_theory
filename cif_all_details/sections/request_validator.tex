\section{Request Validator}
\subsection{Overview}

The Request Validator is a purely combinational block instantiated once
each on the read and write paths. It inspects the head entry of the AR or AW
Metadata FIFO, performs a runtime address range check (and, on the read
path, drives the same-line read/write hazard gate of
\S\ref{sec:samelinehaz}), and steers the transaction to one of two
downstream paths:

\begin{itemize}
  \item \textbf{Valid path:} Transaction forwarded to ROB allocation
        with \texttt{REQ\_VALID} asserted.
  \item \textbf{Error path:} Transaction forwarded to the Error Handler
        with \texttt{ERR\_VALID} asserted and \texttt{AXID} preserved.
\end{itemize}

The Request Validator contains a Configuration Register that stores the valid physical address range used for runtime address validation. 
Apart from this internal configuration state, the Request Validator contains no per-transaction storage or counters; the read-path hazard gate is likewise combinational (\S\ref{sec:samelinehaz}).
All protocol-level checks (burst type, WRAP legality, alignment, and burst
length) are enforced as design-time assertions in the verification
environment only; compliant AXI4 masters are assumed at runtime. Both paths
permit \texttt{AxSIZE = 0..6}, \texttt{INCR}/\texttt{WRAP}/\texttt{FIXED}, and
unaligned first transfers where AXI4 permits them. On the \emph{read} path
narrow/unaligned transfers are resolved by the Burst Splitter's per-line
coalescing (\S\ref{sec:splitter}) and the Read Serializer
(\S\ref{sec:read-serializer}). On the \emph{write} path each AXI write beat is
one MC packet carrying \texttt{WDATA}/\texttt{WMASK}, so the only additional
write-path assertion is \texttt{AWLEN + 1 $\le$ 64} (the per-transaction MC
packet bound, \S\ref{sec:overview}); this replaces the earlier
\texttt{AWSIZE = 6} assertion.

% -----------------------------------------------------------------------
\subsection{Design Parameters}

\begin{table}[h!]
\centering
\begin{tabular}{lll}
\toprule
\textbf{Parameter} & \textbf{Value} & \textbf{Description} \\
\midrule
\texttt{ADDR\_W}  & 32 & Address width in bits \\
\texttt{AXID\_W}  & 6  & AXI ID width \\
\texttt{LEN\_W}   & 8  & Burst length field width (ARLEN/AWLEN) \\
\bottomrule
\end{tabular}
\caption{Request Validator parameters}
\end{table}

% -----------------------------------------------------------------------
\subsection{Internal Blocks}

Configuration Register
Stores the valid physical address range used during request validation.
Provides the configured address range to the Address Range Check block.
Represents the only persistent state within the Request Validator.

\subsection{Address Range Check}

\begin{itemize}
  \item \textbf{Type:} Purely combinational.
  \item \textbf{Input:} \texttt{ADDR[31:0]} from the AR/AW Metadata
        FIFO head.
  \item \textbf{Function:} Checks whether \texttt{ADDR} falls within
        the valid physical address range stored in the Configuration Register.
  \item \textbf{Output:} \texttt{addr\_ok} (1-bit) — asserted when the
        address is valid, deasserted on an address error.
  \item \textbf{Scope:} Address range check only. No burst type, no
        alignment, no length checks at runtime.
\end{itemize}

\subsection{Output Mux}

\begin{itemize}
  \item \textbf{Type:} Purely combinational.
  \item \textbf{Function:} Routes the transaction to the correct
        downstream block based on \texttt{addr\_ok}.
  \item \textbf{Valid path} (\texttt{addr\_ok == 1}): asserts
        \texttt{REQ\_VALID} to ROB allocation; forwards
        \texttt{AXID[5:0]}.
  \item \textbf{Error path} (\texttt{addr\_ok == 0}): asserts
        \texttt{ERR\_VALID} to the Error Handler; forwards
        \texttt{AXID[5:0]}.
  \item Only one output path is active per transaction. Both paths are
        never asserted simultaneously.
\end{itemize}

% -----------------------------------------------------------------------
\subsection{Interface Signals}

\subsection{Inputs}

\begin{longtable}{llll}
\toprule
\textbf{Signal} & \textbf{Width} & \textbf{Source} & \textbf{Description} \\
\midrule
\texttt{ADDR[31:0]}        & 32 & AR/AW Metadata FIFO & Transaction address \\
\texttt{AXID[5:0]}         & 6  & AR/AW Metadata FIFO & AXI ID \\
\texttt{ARLEN/AWLEN[7:0]}  & 8  & AR/AW Metadata FIFO & Burst length \\
\texttt{ARSIZE/AWSIZE[2:0]} & 3 & AR/AW Metadata FIFO & Transfer size (\texttt{0..6}); feeds the 64\,B line footprint calc (\S\ref{sec:samelinehaz}) and the Burst Splitter per-beat/per-line layout (\S\ref{sec:splitter}) \\
\texttt{ARBURST/AWBURST[1:0]} & 2 & AR/AW Metadata FIFO & Burst type; feeds the footprint calc (\S\ref{sec:samelinehaz}) \\
\texttt{STALL}             & 1  & ROB Allocation      & Per-AXID stall; holds output until deasserted \\
\texttt{HAZ\_HOLD}         & 1  & Same-line hazard gate & Read instance only. Head read overlaps an occupied \texttt{ROB\_WR} footprint (\S\ref{sec:samelinehaz}); behaves as \texttt{STALL} --- holds output, no FIFO pop \\
\bottomrule
\caption{Request Validator input signals}
\end{longtable}

\subsection{Outputs}

\begin{longtable}{llll}
\toprule
\textbf{Signal} & \textbf{Width} & \textbf{Destination} & \textbf{Description} \\
\midrule
\texttt{REQ\_VALID}       & 1 & ROB allocation & Valid transaction request \\
\texttt{AXID[5:0]}        & 6 & ROB allocation & Forwarded AXI ID \\
\texttt{ERR\_VALID}       & 1 & Error Handler & Address error notification \\
\texttt{AXID[5:0]}        & 6 & Error Handler & AXI ID preserved for error-response handling \\
\bottomrule
\caption{Request Validator output signals}
\end{longtable}

% -----------------------------------------------------------------------
\subsection{Functional Behaviour}

\begin{enumerate}
  \item Request Validator reads, without popping, the AR/AW Metadata FIFO
        head entry (\texttt{ADDR}, \texttt{AXID}, \texttt{ARLEN/AWLEN},
        \texttt{ARSIZE/AWSIZE}, \texttt{ARBURST/AWBURST}; widths in the
        Inputs table).
  \item Address Range Check compares the incoming address against the physical address range stored in the Configuration Register and evaluates \texttt{addr\_ok} combinationally.
  \item If \texttt{addr\_ok == 1} and \texttt{STALL} is deasserted (and, on
        the read path, \texttt{HAZ\_HOLD} is deasserted):
        Output Mux asserts \texttt{REQ\_VALID} to ROB allocation
        with \texttt{AXID}, and the FIFO head is popped in that cycle
        --- the transaction has been accepted.
  \item If \texttt{addr\_ok == 0}: Output Mux asserts \texttt{ERR\_VALID}
        to the Error Handler with \texttt{AXID}, and the FIFO head is
        popped when \texttt{ERR\_VALID} is asserted. The Error Handler
        drives \texttt{alloc\_req} and, at Metadata-RAM initialization,
        force-fills \texttt{completion\_bitmap} and sets
        \texttt{resp\_status = SLVERR}; the entry is immediately
        retirement-eligible. No packet is emitted to the CIF--MC interface
        (\S\ref{sec:cifmc}, error transactions).
  \item If \texttt{STALL} is asserted (ROB Allocation unavailable for this
        AXID), or \texttt{HAZ\_HOLD} is asserted on the read path
        (\S\ref{sec:samelinehaz}): the Request Validator holds its outputs
        stable and leaves the current head entry in place. The FIFO is not
        advanced until the transaction is accepted (step 3) or routed to
        the Error Handler (step 4).
\end{enumerate}

% -----------------------------------------------------------------------
\subsection{Read/write same-line hazard gate}

Read instance only.  Before \texttt{alloc\_req}, the head read's 64\,B line
footprint --- computed combinationally from
\texttt{\{ARADDR, ARLEN, ARSIZE, ARBURST\}} by the calculation in
\S\ref{sec:samelinehaz} --- is tested for interval overlap against every
occupied \texttt{ROB\_WR} footprint (\texttt{wr\_line\_base},
\texttt{wr\_line\_count}, occupied bit).  On any overlap the gate asserts
\texttt{HAZ\_HOLD}, which holds the Request Validator output and the AR
Metadata FIFO exactly as \texttt{STALL} does.  The overlap result is
re-evaluated whenever a write \texttt{ROB\_INDEX} is freed
(\texttt{free\_req}).  The read admits when it overlaps no occupied write
footprint.  No read-side footprint is stored; see \S\ref{sec:samelinehaz} for
the full description and the MC-contract dependency.

% -----------------------------------------------------------------------
\subsection{Timing Notes}

\begin{itemize}
  \item Fully combinational. Output (\texttt{REQ\_VALID} or
        \texttt{ERR\_VALID}) is valid in the same cycle as the FIFO
        head entry is presented.
  \item \texttt{STALL} from ROB allocation is also combinational —
        the combined path (FIFO output → Address Range Check → Output
        Mux → ROB Allocation Stall Logic → STALL back) is a
        combinational loop that must be timed carefully. A register
        stage may be inserted between the Request Validator output and
        the ROB Allocation if timing closure requires it, at the
        cost of one cycle of allocation latency.
  \item The Address Range Check remains purely combinational. 
        The Configuration Register provides the address range used during validation and is treated as stable during normal request processing.
\end{itemize}

% -----------------------------------------------------------------------
\subsection{Edge Cases}

\begin{itemize}
  \item \textbf{Error path stall:} An address error transaction still
        consumes a ROB allocation. If the AXID is at maximum
        outstanding (\texttt{STALL} asserted), the error path is also
        held. \texttt{ERR\_VALID} is not asserted until \texttt{STALL}
        deasserts.
  \item \textbf{Read held with allocation free:} On the read path a valid
        transaction may be held at the FIFO head by \texttt{HAZ\_HOLD}
        (\S\ref{sec:samelinehaz}) even when \texttt{STALL} is deasserted.
        The write path is unaffected.
  \item \textbf{Protocol violations:} Illegal burst type, unaligned
        \texttt{WRAP}, maximum burst length violations, and a write with
        \texttt{AWLEN + 1 > 64} MC packets are not checked at runtime. They
        are enforced as design-time assertions in the verification
        environment. Narrow (\texttt{AxSIZE < 6}) and unaligned-first-transfer
        reads \emph{and} writes are legal AXI4 and are supported
        (\S\ref{sec:splitter}); they are not violations. A non-compliant
        master will cause undefined behaviour.
  \item \textbf{Simultaneous valid and error:} Not possible —
        \texttt{addr\_ok} is a single-bit result; exactly one output
        path is active per transaction.
\end{itemize}