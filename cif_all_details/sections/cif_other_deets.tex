...
bits.  A write MC packet is one AXI write beat (any \texttt{AWSIZE}), carrying
\texttt{WDATA}/\texttt{WMASK}; a read MC packet is one distinct 64-B line that
may source several AXI read beats (\S\ref{sec:identity},
\S\ref{sec:respformat}, \S\ref{sec:splitter}).
...
AW & \texttt{AWVALID/AWREADY} & 1/1 & Write address handshake\\
   ...
   & \texttt{AWLEN}           & 8   & \texttt{LEN\_W}; raw AXI burst length. One MC packet per AXI write beat, so \texttt{exp\_pkts = AWLEN + 1} (\S\ref{sec:identity}); design-time assertion \texttt{AWLEN + 1 $\le$ 64}\\
   & \texttt{AWSIZE/AWBURST}  & 3/2 & AXI4-fixed; \texttt{AWSIZE = 0..6} supported. Feed the same-line hazard footprint (\S\ref{sec:samelinehaz}) and the Burst Splitter per-beat address/line mapping (\S\ref{sec:splitter})\\
\midrule
W  & \texttt{WVALID/WREADY}   & 1/1 & Write data handshake\\
   & \texttt{WDATA}           & 512 & One AXI write beat per MC request packet; a narrow/unaligned beat occupies its natural byte lanes (\S\ref{sec:identity})\\
   & \texttt{WSTRB}           & 64  & AXI4-fixed (one bit per \texttt{WDATA} byte); carried verbatim as the \texttt{wmask} field on \texttt{wdata\_afifo}. Not all-ones (narrow/unaligned/partial) $\Rightarrow$ MC RMW (\S\ref{sec:cifmc})\\
...
AR & \texttt{ARVALID/ARREADY} & 1/1 & Read address handshake\\
   ...
   & \texttt{ARLEN}           & 8   & \texttt{LEN\_W}; raw AXI burst length. Reads: \texttt{exp\_pkts} is the distinct-64-B-line count (\S\ref{sec:splitter}), not \texttt{ARLEN + 1}; the Read Serializer emits \texttt{ARLEN + 1} R beats\\
   & \texttt{ARSIZE/ARBURST}  & 3/2 & AXI4-fixed; \texttt{ARSIZE = 0..6} supported. Feed the same-line hazard footprint (\S\ref{sec:samelinehaz}), the Burst Splitter line-count/layout (\S\ref{sec:splitter}), and the Read Serializer beat reconstruction (\S\ref{sec:read-serializer})\\
\midrule
R  & \texttt{RVALID/RREADY}   & 1/1 & Read data handshake\\
   & \texttt{RDATA}           & 512 & One 64-B line, driven directly from the returned RD\_SRAM line; no lane realignment (\S\ref{sec:read-serializer})\\
   & \texttt{RID}             & 6   & \texttt{AXID\_W}; from the retiring transaction's Slot FIFO context\\
   & \texttt{RRESP}           & 2   & AXI4-fixed; per-beat value $=$ the per-line status of the 64-B line the beat is sourced from (\S\ref{sec:read-serializer})\\
   & \texttt{RLAST}           & 1   & Asserted on the final AXI beat of the transaction (\texttt{beat\_index == ARLEN}); not derived from \texttt{PACKET\_NUMBER}/\texttt{exp\_pkts} (\S\ref{sec:read-serializer})\\
\bottomrule
...
\texttt{exp\_pkts} & 7 & Splitter $\rightarrow$ RD/WR ROB & Number of MC packets, 1--64: write $=$ \texttt{AWLEN + 1} (one per AXI write beat); read $=$ distinct 64-B lines touched (\S\ref{sec:splitter}). Initializes the ROB's \texttt{exp\_pkts}.\\
...
\texttt{emit\_addr} & 32 & Splitter $\rightarrow$ Packet Formation & 64-B line address of this MC packet (write: the beat's line; read: the touched line).\\
\texttt{PACKET\_NUMBER} & 6 & Splitter $\rightarrow$ Packet Formation & Current \texttt{pkt\_idx} (\texttt{0..exp\_pkts-1}), assigned at packet emission; write $=$ AXI beat index, read $=$ 64-B line index.\\
...
\texttt{rdata} & 512 & MC Core $\rightarrow$ Read ROB & Read data payload, one complete 64-B line; valid only when \texttt{resp\_type = RD\_DATA} (\S\ref{sec:respformat}).\\
\bottomrule
\end{longtable}
...
\textbf{Signal} & \textbf{Width} & \textbf{Path} & \textbf{Description}\\
\midrule
\texttt{merge\_ready} & 1 & Merge Logic $\rightarrow$ Read ROB & Accepts an emitted AXI read beat (\texttt{ARLEN + 1} per transaction).\\
\texttt{rob\_last} & 1 & Read ROB $\rightarrow$ Merge Logic & Asserted with the final AXI read beat (\texttt{beat\_index == ARLEN}) (\S\ref{sec:read-serializer}).\\
...
The write ROB aggregates packet status before its one BRESP; the read path
serializes each line's status onto the AXI beats sourced from that line
(\S\ref{sec:read-serializer}).  The \texttt{status} encoding
