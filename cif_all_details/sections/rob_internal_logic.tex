...
\texttt{PACKET\_NUMBER} & 6 bits (64-B MC packet index, \texttt{0..63}; write: AXI beat index, read: 64-B line index) \\
\midrule
...
\begin{itemize}[noitemsep]
  \item \textbf{Retiring Register} (generic, minimal addition): a single register per instance holding the \texttt{ROB\_INDEX} currently being drained, plus read-instance progress state.
For reads this progress state must track the AXI beat position
(\texttt{beat\_index}, \texttt{0..ARLEN}, up to 256 values) in addition to
the current 64-B line, since a read drains \texttt{ARLEN + 1} beats over
\texttt{exp\_pkts} $\le$ 64 lines (\S\ref{sec:read-serializer}); its exact
width and layout are subject to ROB reconciliation.  Any within-beat
progress state required by the MC/Data-Buffer interface contract is also
held here.
The write instance requires no analogous data-stream progress state. At most one transaction is draining at a time per instance, consistent with the single response-channel path each instance already serves.
  ...
  \item Maximum one transaction draining at a time per instance; draining a single read transaction spans \texttt{ARLEN + 1} AXI R beats (up to 256), which the Read Serializer reconstructs from the transaction's \texttt{exp\_pkts} $\le$ 64 returned 64-B lines (\S\ref{sec:read-serializer}), unlike the previous single-cycle-emit assumption.
\end{itemize}
...
\begin{itemize}[noitemsep]
  ...
  \item \texttt{exp\_pkts} for a read is the exact count of distinct 64-B lines the burst touches (\S\ref{sec:splitter}), \texttt{1..64}; \texttt{PACKET\_NUMBER} is the line index, not the AXI beat index.
  \item \textbf{Read Serializer metadata dependency.} Reconstructing the AXI R-channel beat stream (\S\ref{sec:read-serializer}) additionally requires retained AXI burst information --- \texttt{ARLEN}, \texttt{ARSIZE}, \texttt{ARBURST}, and a normalized base / \texttt{WrapBoundary} --- available to retirement for the life of the entry. Whether this is held in Metadata RAM, a side structure, or Slot-FIFO context is \textbf{subject to ROB reconciliation} and is not decided here.
\end{itemize}
...
The architectural behavior is specified in \S\ref{sec:read-serializer}; its
interaction with retirement is:
\begin{itemize}[noitemsep]
  \item Operates only while the Retiring Register (\S\ref{sec:retire}) holds a read transaction.
  \item Emits \texttt{ARLEN + 1} AXI R beats in order, tagged with the AXID selected from the Slot FIFO context.  For beat \texttt{beat\_index} it forms the AXI beat address per \texttt{ARBURST}/\texttt{ARSIZE} (\texttt{WRAP} relative to \texttt{WrapBoundary}), maps it to a 64-B line index \texttt{PACKET\_NUMBER} (\texttt{0..exp\_pkts-1}), and drives \texttt{RDATA} from the complete line at \texttt{rd\_dbuf\_base + PACKET\_NUMBER}.  Byte lanes are congruent; no barrel shifter is used.  Consecutive beats may re-use the same line.  The within-beat handshake to the merge path follows the MC/Data-Buffer interface contract.
  \item \texttt{merge\_ready} accepts the AXI beat currently presented (\texttt{ARLEN + 1} per transaction).  The contiguous \texttt{rd\_dbuf\_base} range is released as a single unit once the final beat has been accepted.
  \item \texttt{rob\_last} is asserted on the final AXI beat, i.e.\ when \texttt{beat\_index == ARLEN} is accepted --- not on the final line.
  \item Per-beat \texttt{RRESP} is the per-line \texttt{status} of the line the beat is sourced from.
...
\midrule
\texttt{data} & 512 & in, MC Core & Read response data, one complete 64-B line per completed MC packet \\
\texttt{data} & 512 & out, Merge Logic & In-order read response data, one 64-B line per AXI R-channel beat (a line may feed several beats) \\
\texttt{status[3:0]} & 4   & out, Merge Logic & Per-line status of the line the emitted beat is sourced from \\
\texttt{rob\_last}   & 1   & out, Merge Logic & Asserted on the final AXI beat (\texttt{beat\_index == ARLEN}) \\
...
\begin{enumerate}[noitemsep]
  ...
  \item (read instance) Read Serializer emits \texttt{ARLEN + 1} AXI R beats, each sourced from the returned 64-B line \texttt{rd\_dbuf\_base + PACKET\_NUMBER} its address maps to (\S\ref{sec:read-serializer}).
...
Retiring Register         & 1 per instance: \texttt{ROB\_INDEX[7:0]} + read progress state (AXI \texttt{beat\_index} \texttt{0..ARLEN} plus current line; exact width subject to ROB reconciliation, \S\ref{sec:read-serializer}) + within-beat progress state \\
Max transactions draining & 1 per instance at a time; a single read transaction's drain spans \texttt{ARLEN + 1} AXI beats (up to 256) over \texttt{exp\_pkts} $\le$ 64 returned lines \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}[noitemsep]
  ...
  \item \textbf{Read Serializer AXI metadata retention.} Narrow/unaligned read serialization (\S\ref{sec:read-serializer}) requires \texttt{ARLEN}, \texttt{ARSIZE}, \texttt{ARBURST}, and a normalized base / \texttt{WrapBoundary} to be retained and available to retirement for the entry's lifetime. The required \emph{behavior} is fixed; the storage location (Metadata RAM extension, side structure, or Slot-FIFO context) is deferred to the separate ROB reconciliation and is not chosen here.
...
\begin{itemize}[noitemsep]
  ...
  \item Draining a read transaction is explicitly multi-cycle through the Retiring Register.  It emits \texttt{ARLEN + 1} AXI beats sourced from \texttt{exp\_pkts} $\le$ 64 returned lines, so Retirement Logic and the Read Serializer must be read as a small state machine (idle / retiring), not a single combinational selection. The write instance is analogous but single-beat, gated by \texttt{bresp\_ready}.
\end{itemize}